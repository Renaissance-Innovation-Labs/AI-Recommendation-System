...
import loadingicon from '../categories/images/loading icon.gif'
    ...
    function MovieCategoryPage() {
        ...
        const { isLoading } = useMovieRecommendation()
        ...
        return (
            <div className="p-3 space-y-6 w-full">
                {
                    isLoading ? (<div className="w-full min-h-screen flex items-center  justify-center"><img className="mx-auto text-center w-44 h-44 " src={loadingicon} alt="loading icon" /> </div>)
                        :
                        (
                            <div>
                                ...
                                <div className="flex flex-col gap-3 md:flex-row">
                                    <h1 className="py-3">Want more recommendations? Try another genre!</h1>
                                    <Link to='/home'>
                                        <button className="bg-slate-700 text-white  mt-2 p-2 md:text-md">Return to HomePage</button>
                                    </Link>
                                </div>
                            </div>
                        )}            
